...
/**
...

    {
        ...
        fprintf(stderr, "%s: %s sqlite: 操作失败！错误编码：%d \n", SERVER_NAME, ERROR_MSG,ret);
   ...
   char *sql= "Create TABLE IF NOT EXISTS [files] ([file] VARCHAR(128),[hash] VARCHAR(128), [size] INT,[peer] VARCHAR(128))";
   char *zErrMsg;
   int ret= sqlite3_exec( db , sql , 0 , 0 , &zErrMsg );
   if(ret!=SQLITE_OK){
